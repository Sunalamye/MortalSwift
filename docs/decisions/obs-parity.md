...
**日期**: 2026-08-01
**狀態**: 已定案（選 B）；已移植區段完成對拍，單人期望值表與役種判定待補
...
已移植區段 = channel 0–876 與 878–888，共 **888 格逐格一致**。
其餘 **124 格**（ch877 與 ch889–1011）需要單人期望值表與役種判定，尚未移植，
被排除在驗收之外——其中在測試劇本裡實際有值而對不上的是 63 格，
另外 61 格兩邊都是 0，但那**不等於已驗證**，只代表這個劇本沒把它們寫到。
...
| Channel | 內容 | 需要 |
|---------|------|------|
| ch877 | 打了之後無條件聽牌且有役的候選 | `AgariCalculator::has_yaku`（agari.rs，45KB） |
| ch889–1011 | 單人期望值表：每張打牌在每一巡的聽牌率／和牌率／期望值 | `algo/sp`（約 50KB Rust） |

這 123 格在 Swift 版**保留正確的格數但填 0**。格數對是關鍵：
內容是 0 只是資訊缺失，格數錯則是其後全部語意錯位。

同一個缺口也讓副露手的自摸判定只能樂觀當作「有役」——
實際送出前還有伺服器 oplist 這一層把關（見 Naki 的 AUDIT §12.2）。
